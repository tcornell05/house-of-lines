const getEulerianPaths = (graph) => {
    const startingVertices = findStartingVertices(graph);
    //Find euler paths for each vertices
    return startingVertices.map((vertex) => {
        const paths = findPaths(vertex, [], {...graph});

        // Note: `paths` is the result of a generator (aka iterator),
        const pathsArr = [...paths];
        return pathsArr.map((path) => path);
    });
...
function* findPaths(vertex, path, graph){
    ...
    // Check if path is eulerian (Are all edges visited once?)
...
module.exports.getEularianPaths = getEulerianPaths;